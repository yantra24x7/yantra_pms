...
int main(char **argv, int argc)
{
	 ...
	 /* if (EW_OK != cnc_startupprocess (1, "fanuc.log"))
    		{
		...
		for (i = 0; i < 1 ; i++)
       	{
			...
			else
			{
				...
				mctimer3.minute=0;
				mctimer3.msec=0;
				mcaxis[0].svload.data=0;
				mcaxis[1].svload.data=0;
				mcaxis[2].svload.data=0;
				mcspindle[0].spload.data=0;
				mcspeed.acts.data=0;
				mcspeed.actf.data=0;
			}	
...
//----------------------------------------------------------------------------------------------------------------------
			...
			Previous_status=s;
			//printf("in from for loop");
		}
		 if(l=1);
		   l=-1;
			//printf("out from for loop");
